...
import {getLastFeedHours} from "../../helpers/functions";
...
const Status = ({bg, type}) => {
    ...
    console.log(status.lastFeedTime);
    let last_feed_before = getLastFeedHours(status.lastFeedTime);

    if (last_feed_before > 48)
        last_feed_before = 48;

    if (last_feed_before < 0)
        last_feed_before = 0;

    console.log(last_feed_before);
    return (
                ...
                <div className={"container" + (type === "wide" ? "-fluid" : "")}>
                    <div className="row">
                        ...
                        >
                            ...
                            >
                                <VisibilitySensor onChange={viewChangeHandler} delayedCall>
                                    <CountUp end={viewed ? last_feed_before : 0}/>
                                </VisibilitySensor>
                                {" h"}
                            </h2>
                            <h3 className={bg === "white-bg" ? "dark-color" : ""}>
